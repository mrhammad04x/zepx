...
function CheckOut() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate form
        if (!editdata.first_name || !editdata.last_name || !editdata.address || 
            !editdata.email || !editdata.contact || !paymentMethod) {
            setError("Please fill in all required fields and select a payment method");
            return;
        }
        
        if (paymentMethod === "online") {
            handlePayment(orderSummary.total);
        } else {
            // Handle cash on delivery or wallet payment
            try {
                // Here you would process the order in your backend
    ...
    const handlePayment = async (price) => {
        try {
            ...
            const response = await axios.post(`${port}/create-order`, {
                amount: Math.round(price * 100), // Ensure amount is an integer in paise
                currency: "INR",
            });
            ...
            // Add a check to verify the Razorpay SDK is loaded
            if (window.Razorpay) {
                                    ...
                                    <div className='chekout-address-information'>
                                            ...
                                            <div className='chekout-name'>
                                                ...
                                                <div className='checkout-names'>
                                                    <input
                                                        ...
                                                        // required
                                                    />
                                                    <input type='text' placeholder='Last name' id="lastName" name="last_name" value={editdata.last_name || ""} onChange={handleChange} />
                                                </div>
                                            </div>
                                        ...
                                        <div className='checkout-conatct'>
                                            <div className='checkout-email checkout-conatct-info'>
                                                ...
                                                <div className='div'>
                                                    <input type='email' placeholder='example@gmail.com' id="email" name="email" value={editdata.email || ""} onChange={handleChange} required />
                                                </div>
                                            </div>
                                            <div className='checkout-number checkout-conatct-info'>
                                                ...
                                                <div className='div'>
                                                    <input type='tel' placeholder='ex.1234567890' value={editdata.contact || ""} id="contact" name="contact" onChange={handleChange}  />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
